...
import numpy as np
import gym
from CartPole_ActorCritic import ActorCritic
...
env_name = 'A2C_CartPole_' + time.ctime(time.time())
env = gym.make('CartPole-v1')#two_step_task()
...
def train():
    ...
    for ep in range(MAX_EP):
        ...
        while not d:
            step += 1
            #print(s, a, r, t)
            #s = np.concatenate([s,[0]]) #s + a + [r] + [t]#[s, a, r, step/100.0]
            #print("STATE : ", s)
        ...
        # critic loss
        value_loss = F.mse_loss(state_values.squeeze(-1), returns).sum()
        ...
        if ep % int(ENV_RESET_TERM) == 0 and ep != 0:
