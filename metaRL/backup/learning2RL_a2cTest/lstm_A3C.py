...
env = MAB()# for i in range(5001)]#gym.make(env_name)
...
def train():
    ...
    for idx in range(MAX_EP//RESET_TERM):
        ...
        for ep in range(RESET_TERM):
            ...
            state_lstm = local_policy.reset_lstm()
            if ep % 10 == 0:
                #print("env created")
                env = MAB(n=k) 
                #print("Prob : ", env.prob)
            while not d:
                ...
                one_hot_action = [1-a, a]
                s = one_hot_action + [r, step/100]#, lstm_a, lstm_r]#env.reset() 
                s = torch.FloatTensor(s).to(device).unsqueeze(0)

                state_pred, action_pred, state_lstm = local_policy(s, state_lstm)
            ...
            cnt += 1
            if ep % 10 == 0:
                print("Try - {} | Env - {} => Mean Reward : {}".format(idx, ep, np.mean(train_reward[-10:])))
                #train_reward = []
        break    
